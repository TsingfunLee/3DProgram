...
/** 载入MD2文件 */
bool CMD2Loader::ImportMD2(t3DModel_MD2 *pModel, char *strFileName, char *strTexture)
{
...
/** 分析并获取帧信息 */
void CMD2Loader::ParseAnimations(t3DModel_MD2 *pModel)
{
...
/** 将各帧数据保存在模型结构中 */
void CMD2Loader::ConvertDataStructures(t3DModel_MD2 *pModel)
{
	int j = 0, i = 0;

	/** 分配内存 */
	memset(pModel, 0, sizeof(t3DModel_MD2));
	...
	for (i=0; i < pModel->numOfObjects; i++)
	{
		...
		currentFrame.pFaces    = new tFace_MD2 [currentFrame.numOfFaces];
...
/** 返回时间t,用于在两帧之间进行插值 */
float CMD2Loader::ReturnCurrentTime(t3DModel_MD2 *pModel, int nextFrame)
{
...
void CMD2Loader::AnimateMD2Model()
{
	...
	/** 获取当前帧信息 */
	t3DObject_MD2 *pFrame =		 &m_3DModel.pObject[m_3DModel.currentFrame];

	/** 获取下一帧信息 */
	t3DObject_MD2 *pNextFrame =  &m_3DModel.pObject[nextFrame];

	/** 获取第一帧信息 */
	t3DObject_MD2 *pFirstFrame = &m_3DModel.pObject[0];
