...
void C3DSLoader::Draw() 
{
	...
	for(int i = 0; i < m_3DModel.numOfObjects; i++)
	{
		...
		t3DObject_3DS *pObject = &m_3DModel.pObject[i];/**< 获得当前显示的对象 */
...
/** 载入3DS文件到模型结构中 */
bool C3DSLoader::Import3DS(t3DModel_3DS *pModel, char *strFileName)
{	
...
/** 读出3ds文件的主要部分 */
void C3DSLoader::ReadNextChunk(t3DModel_3DS *pModel, tChunk *pPreChunk)
{	
	t3DObject_3DS newObject = {0};					/**< 用来添加到对象链表 */
	...
	{	
		...
		case OBJECT:							
			...
			/** 初始化对象和它的所有数据成员 */
			memset(&(pModel->pObject[pModel->numOfObjects - 1]), 0, sizeof(t3DObject_3DS));
...
/** 处理所有的文件中对象的信息 */
void C3DSLoader::ReadNextObjChunk(t3DModel_3DS *pModel, t3DObject_3DS *pObject, tChunk *pPreChunk)
{	
...
/** 处理所有的材质信息 */
void C3DSLoader::ReadNextMatChunk(t3DModel_3DS *pModel, tChunk *pPreChunk)
{	
	int buffer[50000] = {0};					/**< 用于读入不需要的数据 */
	/** 给当前块分配存储空间 */
	m_CurrentChunk = new tChunk;
...
/** 读入顶点索引 */
void C3DSLoader::ReadVertexIndices(t3DObject_3DS *pObject, tChunk *pPreChunk)
{	
	unsigned short index = 0;					/**< 用于读入当前面的索引 */
	/** 读入该对象中面的数目 */
	pPreChunk->bytesRead += fread(&pObject->numOfFaces, 1, 2, m_FilePointer);
	/** 分配所有面的存储空间，并初始化结构 */
	pObject->pFaces = new tFace_3DS [pObject->numOfFaces];
	memset(pObject->pFaces, 0, sizeof(tFace_3DS) * pObject->numOfFaces);
	/** 遍历对象中所有的面 */
...
/** 读入对象的UV坐标 */
void C3DSLoader::ReadUVCoordinates(t3DObject_3DS *pObject, tChunk *pPreChunk)
{	
	/** 读入UV坐标的数量 */
...
/**  读入对象的顶点 */
void C3DSLoader::ReadVertices(t3DObject_3DS *pObject, tChunk *pPreChunk)
{	
	/** 读入顶点的数目 */
...
/** 读入对象的材质名称 */
void C3DSLoader::ReadObjMat(t3DModel_3DS *pModel, t3DObject_3DS *pObject, tChunk *pPreChunk)
{	
...
/** 计算对象的法向量 */
void C3DSLoader::ComputeNormals(t3DModel_3DS *pModel)
{	
	...
	{	
		/** 获得当前的对象 */
		t3DObject_3DS *pObject = &(pModel->pObject[index]);
