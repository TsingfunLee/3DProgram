...
{	int numOfObjects;			        /**< 模型中对象的数目 */
	...
	vector<t3DObject_MD2> pObject;	        /**< 模型中对象链表信息 */
};
...
public:
    ...
    /** 返回3D模型结构 */
	t3DModel_MD2& GetModel() { return m_3DModel; }
	...
	/** 模型结构 */
	t3DModel_MD2                m_3DModel;                  
	
private:
	
	/** 载入MD2文件 */
	bool ImportMD2(t3DModel_MD2 *pModel, char *strFileName, char *strTexture);
	...
	/** 保存动作信息 */
	void ParseAnimations(t3DModel_MD2 *pModel);

	/** 将MD2文件结构转化为模型结构 */
	void ConvertDataStructures(t3DModel_MD2 *pModel);

	/** 释放内存 */
	void CleanUp();

	/** 返回当前时间 */
	float ReturnCurrentTime(t3DModel_MD2 *pModel, int nextFrame);
