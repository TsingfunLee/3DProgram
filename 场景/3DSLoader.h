...
{	int numOfObjects;			  /**< 模型中对象的数目 */
	...
	vector<t3DObject_3DS> pObject;	  /**< 模型中对象链表信息 */
};
...
public:
	...
	/** 装载3ds文件到模型结构中 */
	bool Import3DS(t3DModel_3DS *pModel, char *strFileName);
...
private:
	...
	/** 读取下一个块 */
	void ReadNextChunk(t3DModel_3DS *pModel, tChunk *);

	/** 读取下一个对象 */
	void ReadNextObjChunk(t3DModel_3DS *pModel,t3DObject_3DS *pObject,tChunk *);

	/** 读取下一个材质块 */
	void ReadNextMatChunk(t3DModel_3DS *pModel, tChunk *);	

	/** 读取对象颜色的RGB值 */
	void ReadColor(tMatInfo *pMaterial, tChunk *pChunk);

	/** 读取对象的顶点信息 */
	void ReadVertices(t3DObject_3DS *pObject, tChunk *);

	/** 读取对象的面信息 */
	void ReadVertexIndices(t3DObject_3DS *pObject,tChunk *);

	/** 读取对象的纹理坐标 */
	void ReadUVCoordinates(t3DObject_3DS *pObject,tChunk *);

	/** 读取赋予对象的材质 */
	void ReadObjMat(t3DModel_3DS *pModel,t3DObject_3DS *pObject,tChunk *pPreChunk);

	/** 计算对象顶点的法向量 */
	void ComputeNormals(t3DModel_3DS *pModel);	
